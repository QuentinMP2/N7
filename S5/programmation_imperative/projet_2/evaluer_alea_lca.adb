...
procedure Evaluer_Alea_LCA is
	...
	begin
		...
		-- Initialiser la LCA avec toutes les clés
		-- pour pouvoir toutes les prendre en compte
		for i in 1..Borne loop
			Enregistrer(LCA_Test, i, 0);
		end loop;

		for i in 1..Taille loop
			begin
				Get_Random_Number(Nb_Aleatoire);
				Enregistrer(LCA_Test, Nb_Aleatoire, La_Valeur(LCA_Test, Nb_Aleatoire) + 1);
				
				exception
					when others => Enregistrer(LCA_Test, Nb_Aleatoire, 1);
			end;
		end loop;

		Pour_Chaque_Min_Max(LCA_Test);
