...
(******************************************************************************)
(*   renvoie la liste des éléments d'un arbre dans l'ordre lexicographique    *)
(*   signature  : list_of_trie : 'a arbre -> 'a list list                     *)
(*   paramètres : un arbre n-aire                                             *)
(*   résultat   : la liste des éléments d'un arbre dans l'odre lexicographique*)
(******************************************************************************)
   ...
   let rec parcours_arbre (Noeud(b, lb)) =
      ...
      | _ -> 
         if b then 
            ...
            List.flatten(List.map parcours_branche lb)
   and parcours_branche (c, abr) =
      List.map (fun x -> c::x) (parcours_arbre abr)
   in parcours_arbre trie
